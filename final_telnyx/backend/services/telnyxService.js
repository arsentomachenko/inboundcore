...
class TelnyxService {
  ...
  async startRecording(callControlId) {
    try {
      console.log(`🎙️  Starting recording for call: ${callControlId}`);
      
      // Use direct REST API to start recording
      // Telnyx record_start action format
      const result = await axios.post(
        `https://api.telnyx.com/v2/calls/${callControlId}/actions/record_start`,
        {
          channels: 'both',  // Record both inbound and outbound audio
          format: 'mp3',     // MP3 format for easy playback
          play_beep: false   // Don't play beep (optional, set to true if you want to notify)
        },
        {
          headers: {
            'Authorization': `Bearer ${process.env.TELNYX_API_KEY}`,
      ...
      console.log(`✅ Recording started successfully for call: ${callControlId}`);
      console.log(`   Response:`, JSON.stringify(result.data, null, 2));
      return result.data;
    } catch (error) {
      ...
      // Don't throw - recording might already be started or call might not support it
      console.warn('⚠️  Continuing despite recording error');
      return null;
    }
  }
  ...
  async stopRecording(callControlId) {
    try {
      console.log(`🛑 Stopping recording for call: ${callControlId}`);
      
      // Use direct REST API to stop recording
      const result = await axios.post(
      ...
      console.log(`✅ Recording stopped successfully for call: ${callControlId}`);
      return result.data;
    } catch (error) {
